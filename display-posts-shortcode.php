...
// THIS WAS ADDED TO PLACE CUSTOM FIELD VALUES IN THE FRONT END 6/19/2019
...
function be_display_posts_custom_fields( $output, $atts, $image, $title, $date, $excerpt ) {
	...
	global $post;
	$cost = esc_attr( get_post_meta( $post->ID, 'Cost', true ) );
	...
	// If the value is not empty, for the custom field, wrap them with <span>'s
    ...
    if( isset( $markings ) ) { 
        if ($markings != '') {
            $markings = '<span class="markings">' . $markings . '</span> ';
        }
    }
